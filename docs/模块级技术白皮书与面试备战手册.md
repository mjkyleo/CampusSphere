...
| **中间件层** | Celery | 异步任务：邮件 / 通知 / 搜索索引同步（4 队列） |
...
- **难点：索引与库的一致性**——写入先落库、再异步同步索引，存在短暂不一致窗口（可接受，最终一致）；同步失败由 Celery 重试兜底，且查询侧有 DB 兜底，不会丢能力。
...
**用了什么技术**
- **4 个队列**（`task_routes`）：`email`（`tasks/email.py` 验证码/通知邮件）、`notify`（`tasks/notify.py` 站内通知）、`search`（`tasks/search_sync.py` 索引同步）、`default`（兜底）。每个队列可独立起 worker、独立扩缩容。
...
- `trade_sessions`：item_id、buyer_id、seller_id、status、conversation_id（可挂接消息会话）
...
**③ 核心接口**
REST：`GET /api/messages/conversations`（会话列表+最后消息+未读数）、`GET /{conv}/`（历史分页）、`POST /{conv}/read`（已读）、`GET /unread`。
...
**③ 核心接口**（`/api/courses/*`）
`GET` 列表（keyword/department 过滤 + 分页）、`GET /departments`（**公开**读取院系列表，school.yaml 兜底）、`GET /{id}` 详情（含评价）、`POST /{id}/reviews`（评课）、`POST` 建课（登录）。管理端：`/api/admin/courses/departments` 读写院系列表。
...
**③ 核心接口**
公开读：`GET /api/canteens`（列表）、`GET /{id}`（详情）、`GET /dishes/{id}`（菜品详情+评价）、`POST /dishes/{id}/reviews`（评菜）。
...
**③ 核心接口**（`/api/jobs/*`）
公开读列表（keyword/status 过滤）；登录：`POST` 发岗、`POST /{id}/apply` 应聘、`GET /{id}/applications` 查看申请（**仅 poster**）。
...
**③ 核心接口**（`/api/shares/*`）
公开读列表（category 过滤）；登录：`POST` 发布、`GET /{id}/download`（预签名下载 + 计数 +1）。
...
**③ 核心接口**（`/api/teams/*`）
公开读列表（仅招募中）、详情（含成员）；登录：`POST` 建队、`POST /{id}/join` 申请加入。
...
| A14 | 搜索怎么做的？ | Meilisearch 中文分词 + Celery 异步同步索引 + 不可用回退 DB LIKE（2.7）。 |
| A15 | 上传文件怎么保证安全？ | 白名单（MIME+扩展名）+ 大小限制 + uuid 对象键 + 预签名直传/本地降级 + 孤儿文件清理（2.6/3.12）。 |
| A16 | 邮件/耗时任务怎么做？ | Celery 4 队列（email/notify/search/default）+ `acks_late` + 重试退避 + broker 快速失败降级内联直发（2.8）。 |
...
| `app/tasks/celery_app.py` 等 | Celery 4 队列 + 兼容补丁 |
