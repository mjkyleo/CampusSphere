...
async def admin_get_item_categories(db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
    """读取二手交易分类（后台配置）。"""
...
async def admin_put_item_categories(
    data: ItemCategoriesConfig, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)
):
    """更新二手交易分类（写 DB，实时生效）。"""
...
async def admin_get_course_departments(db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
    """读取课程开课院系列表（后台配置）。"""
...
async def admin_put_course_departments(
    data: CourseDepartmentsConfig, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)
):
...
async def admin_list_canteens(db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
    """食堂管理列表（含档口与菜品）。"""
...
async def admin_create_canteen(
    data: CanteenCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)
):
...
async def admin_update_canteen(
    canteen_id: str, data: CanteenCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)
):
...
async def admin_delete_canteen(canteen_id: str, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
    """删除食堂（级联删除档口、菜品与评价）。"""
...
async def admin_create_stall(
    data: StallCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)
):
...
async def admin_update_stall(
    stall_id: str, data: StallCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)
):
...
async def admin_delete_stall(stall_id: str, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
...
async def admin_create_dish(
    data: DishCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)
):
...
async def admin_update_dish(
    dish_id: str, data: DishCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)
):
...
async def admin_delete_dish(dish_id: str, db: AsyncSession = Depends(get_db), _=Depends(get_current_admin)):
